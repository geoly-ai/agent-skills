...
| Tool | Purpose | Params |
|---|---|---|
| `get_citation_overview` | The `/sources/citations` domains tab: `{ caliber, window, stats, board }`. **`board` (0.5.3) = the redesigned page, same read model**: `totals {roots, citations}`, `you {rootDomains, citations, share, rank}` ("your site holds X%, ranks #N"), `movers {top, new, up, down}` + `enteredTop20` / `biggestDrop` (closed days vs the previous equal-length window; `prevCitations`/`deltaPct` null = no comparison — 90d never compares, `prevReady=false` = previous window still building), `trend` (top-5 domains + you per day), `typeShare {you, competitor, other}`, `coverage.missingDays`; `status="not-ready"` is a state, not empty data. `stats` = the older KPI block (counts URLs, not records): totals, ownership, top domains, subdomains, daily trend, platform distribution — kept for existing callers; `stats.trend.citations` (half vs half) is deprecated, use `board.movers`. With `topic_ids` / `competitor_ids` the response is `{ board }` only. Window = N whole +08 calendar days on the citation-creation axis + today; `caliber` (`created-day` / `legacy`) + `window` describe `stats` — quote them | `time_range, platform, topic_ids[], competitor_ids[] (get_competitor_list rows' legacy id)` |
| `list_citation_domains` | **The domain TABLE of `/sources/citations`** (0.5.3, same read model as the page): one page of cited root domains with `rank`, `rootDomain`, `domainType`, `citations` (incl. today), `share` (of all citations in the window — denominator ignores filters), `prevCitations` / `deltaPct` / `isNew` (closed days vs previous window; null = no comparison), `selfMentioned` (answers citing this domain also mentioned you; null = co-occurrence layer off → "unknown"), `brands[]` + `brandsExtra` (registered competitors mentioned in answers citing the domain). **`gap_only=true` = the page's "只看缺口" switch** (competitor mentioned, you not, not a competitor's own site) — `gapUnavailable="no-competitors"` means none registered, not "no gaps". `status="not-ready"` / `invalidFilter="competitors"` / `coverage.missingDays` are states to report. One domain's pages / answers → `get_domain_detail` | `time_range, platform, topic_ids[], competitor_ids[], search, gap_only (bool), type: all\|you\|competitor\|other, sort: citations\|delta\|domain\|share, page, page_size (default 20, max 100)` |
...
index (26 current entry points + 3 public deprecated aliases from § Deprecated = 29 registered here; the other 4 deprecated names, `get_competitor_overview`, `get_brand_citations_daily`, `get_ga4_page_data` and `get_content_opportunities`, are brand read-only tools counted in the 40 — 7 deprecated names in total):
...
| Source | Count |
|---|---|
| Read-only (brand + GA4 + public-source + `get_brand_context` + `get_current_date` + `resolve_page_context`) | 40 |
...
> only) and is **not registered at all** — it is not part of the 40. Display sections A–J above
